...
public class OSClient {
	...
	private ImageModel getImageModel(JSONObject image) throws JSONException {
		ImageModel im = new ImageModel();
		im.setId(image.getString("id"));
		im.setName(image.getString("name"));
		JSONArray links = image.getJSONArray("links");
		im.setUrl(links.getJSONObject(0).getString("href"));
		return im;		
	}
	...
	public List<ImageModel> getImageList() {
		...
		try {
			// http://208.123.85.197:8774/v2/tenant_id/servers
			String imageListUrl = "http://" + this.ipAddr + ":8774/v2/"
					+ this.tenantId + "/images";
			JSONObject jo = new JSONObject(GET(imageListUrl));
			
			Object o = jo.get("images");
			if (o instanceof JSONArray) {
				JSONArray ja = (JSONArray) o;
				for (int i = 0; i < ja.length(); i++) {
					result.add(getImageModel(ja.getJSONObject(i)));
				}
			} else if (o instanceof JSONObject) {
				result.add(getImageModel((JSONObject) o));
			}
		}catch(Exception e) {
	...
	public void launchServer(ImageModel im) {
		try {
			int tick = Calendar.getInstance().get(Calendar.SECOND);
			String launchData = "{  \"server\":{     \"name\":\"" + im.getName() + "_" + tick  + "\",     \"imageRef\":\"" + im.getId() + "\",     \"flavorRef\":\"1\",     \"max_count\":1,     \"min_count\":1,     \"networks\":[        {           \"uuid\":\"8659726f-b7f1-4b34-8e01-16e16ac33fcf\"        }     ]  }}";
			String launchUrl = "http://" + this.ipAddr + ":8774/v2/"
					+ this.tenantId + "/servers";
			
			POST(launchUrl, launchData);
